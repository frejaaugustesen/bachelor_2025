...
## marker genes ----
islet38 <- FindNeighbors(islet38, dims = 1:15)
islet38 <- FindClusters(islet38, resolution = 0.6)

## clustering of cells ----
islet38 <- RunUMAP(islet38, dims = 1:15)

DimPlot(islet38, reduction = "umap", label = TRUE)

# Dotplots ----------------------------------------------------------------


DotPlot1 <- DotPlot(islet38, 
...
# cluster 5: ductal
# cluster 6: acinar
# cluster 7: activated stellate
# cluster 8: endothelial
# cluster 9: immune
# cluster 10:cycling/alpha

## klade til annotering ----


islet38@meta.data <- islet38@meta.data %>% dplyr::mutate(manual_anno = dplyr::case_when(
  seurat_clusters %in% c(1) ~ "alpha",
  seurat_clusters %in% c(4) ~ "beta/delta",
  ...
  seurat_clusters %in% c(0, 2, 5) ~ "ductal",
  seurat_clusters %in% c(3) ~ "gamma",
))

DimPlot(islet38, group.by = "manual_anno", 
        reduction = "umap", label = TRUE, repel = TRUE) +
  NoLegend()

DimPlot(islet38, group.by = "seurat_clusters", 
        reduction = "umap", label = TRUE, repel = TRUE) +
  NoLegend()
...
# small dotplots ----------------------------------------------------------
beta1 <- c("IAPP", "INS", "HADH")
delta1 <- c("SST", "RBP4", "PCSK1")
alpha1 <- c("GCG", "TTR", "PEMT")
ductal1 <- c("SERPINA1", "ANXA4", "CFTR")
...
DoHeatmap(islet38, features = c(beta, alpha, delta, gamma, epsilon, cycling, ductal, 
...
FeaturePlot(islet38, features = c("INS", "GCG", "SST", "PPY"), pt.size = 1)
