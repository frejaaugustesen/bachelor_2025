...
motakis_beta_t2d <- subset(x = motakis_beta, idents = "t2d")


## Marker genes ------------------------------------------------------------
Idents(motakis_beta_t2d) <- "subtype"

markergenes_t2d <- FindMarkers(motakis_beta_t2d, ident.1 = "nd", ident.2 = "t2d", 
                               group.by = "subtype")



allmarkers_t2d <- FindAllMarkers(motakis_beta_t2d, only.pos = TRUE, min.pct = 0.1)
...
# Subset to only contain real T2D individuals 
# (Jeg tror måske det er bedre at sammenligne subtyper, for hver disease?)
motakis_beta_nd <- subset(x = motakis_beta, idents = "nd")


## Marker genes ------------------------------------------------------------
Idents(motakis_beta_nd) <- "subtype"

markergenes_nd <- FindMarkers(motakis_beta_nd, ident.1 = "nd", ident.2 = "t2d", 
                              group.by = "subtype")



allmarkers_nd <- FindAllMarkers(motakis_beta_nd, only.pos = TRUE, min.pct = 0.1)
...
Idents(motakis_beta_nd) <- "disease"

# Prepare data for differential gene expression analysis
counts_nd <- motakis_beta_nd %>%
  edgeR::Seurat2PB(sample = "orig.ident",
                   cluster = "subtype")
...
# Subset to only contain real T2D individuals 
# (Jeg tror måske det er bedre at sammenligne subtyper, for hver disease?)
motakis_beta_pre <- subset(x = motakis_beta, idents = "pre")


## Marker genes ------------------------------------------------------------
Idents(motakis_beta_pre) <- "subtype"

markergenes_pre <- FindMarkers(motakis_beta_pre, ident.1 = "nd", ident.2 = "t2d", 
                               group.by = "subtype")



allmarkers_pre <- FindAllMarkers(motakis_beta_pre, only.pos = TRUE, min.pct = 0.1)
...
Idents(motakis_beta_pre) <- "disease"

# Prepare data for differential gene expression analysis
counts_pre <- motakis_beta_pre %>%
  edgeR::Seurat2PB(sample = "orig.ident",
                   cluster = "subtype")
...
## Marker genes ------------------------------------------------------------
Idents(motakis_beta) <- "subtype"

markergenes <- FindMarkers(motakis_beta, ident.1 = "nd", ident.2 = "t2d", 
                               group.by = "subtype")



allmarkers <- FindAllMarkers(motakis_beta, only.pos = TRUE, min.pct = 0.1)
...
norm_counts %>% dplyr::filter(gene == "OXR1") %>%
  ...
  geom_bar(stat='summary', fun = "mean") +
  ggplot2::geom_point()+
  ggtitle("Motakis subtypes, gene OXR1")
